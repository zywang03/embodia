...
- `capture()` / `observe()` 返回 `em.Frame` 或 frame-like `dict`，里面要有
  `images`、`state`。其中数值载体默认就是 numpy：
  `frame.images[*]`、`frame.state[*]` 应该是 `numpy.ndarray`。`images`
  的键来自 `schema.images`，`state` 的键来自所有
  `schema.components[*].state` 的并集。如果你不填 `timestamp_ns`
...
- `send_command(action)` / `act(action)` 接收的是一个 `em.Action`。其中
  ...
  `action.commands[组件名].value` 是 numpy 向量。
...
- `infer(frame)` 接收的是一个 `em.Frame`。`frame.images` 和 `frame.state`
  ...
  其中数值也默认用 numpy 表示，commands 仍然必须对齐到 YAML 里的组件定义。
