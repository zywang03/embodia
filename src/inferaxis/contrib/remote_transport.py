...
This module keeps inferaxis's core package free of websocket and msgpack
dependencies, while still making it easy to talk to a remote policy server
from local Python code.

Payload conversion lives in ``inferaxis.contrib.remote_transform`` so this
...
The remote server can send NumPy arrays in a custom msgpack encoding. inferaxis's
...
def _import_msgpack() -> Any:
    ...
    try:
        ...
        raise InterfaceValidationError(
            ...
            "package. Install it with `pip install 'inferaxis[remote]'`."
        ) from exc
...
class RemoteMsgpackCodec:
    """Msgpack codec for inferaxis's remote ndarray wire format."""
...
class InferaxisPolicyAdapter:
    """Expose an inferaxis-style policy through the remote websocket protocol.
    ...
    def __init__(
        ...
        *,
        ...
        if transform is not None and (
            ...
            raise InterfaceValidationError(
                "InferaxisPolicyAdapter accepts either transform=... or "
    ...
    def get_server_metadata(self) -> dict[str, Any]:
        ...
        inferaxis_meta = metadata.setdefault("inferaxis", {})
        if not isinstance(inferaxis_meta, dict):
            raise InterfaceValidationError(
                "server_metadata['inferaxis'] must be a mapping when provided."
            )

        get_spec, _ = resolve_callable_method(self.policy, POLICY_GET_SPEC_METHODS)
        if callable(get_spec):
            inferaxis_meta.setdefault("policy_spec", policy_spec_to_dict(get_spec()))
        ...
        if self._action_plan_provider is not None:
            ...
            if not callable(infer):
                raise InterfaceValidationError(
                    ...
                    "served through InferaxisPolicyAdapter."
                )
            raw_plan = infer(frame)

        actions = _coerce_inferaxis_action_plan(raw_plan)
...
def serve_policy(
    ...
    *,
    ...
    """Serve one policy-like object through inferaxis's remote websocket API."""
...
class RemotePolicy:
    """Policy-like source that talks to inferaxis's remote policy server.
    ...
    def _default_response_to_action(self, response: object) -> Action:
        """Convert one remote response into a validated inferaxis action."""
    ...
    def _default_response_to_action_plan(self, response: object) -> list[Action]:
        """Convert one remote response into a validated inferaxis action plan."""
    ...
    def _resolve_action_shape(
        ...
        if isinstance(response, Mapping):
            inferaxis_meta = response.get("inferaxis")
            if isinstance(inferaxis_meta, Mapping):
                if target is None:
                    raw_target = inferaxis_meta.get("action_target")
                    if isinstance(raw_target, str) and raw_target.strip():
                        target = raw_target
                if command is None:
                    raw_command = inferaxis_meta.get("action_command")
                    if isinstance(raw_command, str) and raw_command.strip():
                        command = raw_command
                if ref_frame is None:
                    raw_ref_frame = inferaxis_meta.get("action_ref_frame")
                    if raw_ref_frame is not None:
                        if (
                            ...
                            raise InterfaceValidationError(
                                "remote response inferaxis.action_ref_frame "
        ...
        if target is None or command is None:
            raise InterfaceValidationError(
                ...
                "payload. Return inferaxis action metadata from the server, "
    ...
    def inferaxis_bind_robot(self, robot: object) -> None:
    ...
    def get_spec(self) -> Any:
        ...
        inferaxis_metadata = metadata.get("inferaxis")
        if not isinstance(inferaxis_metadata, Mapping):
            raise InterfaceValidationError(
                "remote server metadata does not contain inferaxis.policy_spec."
            )

        raw_spec = inferaxis_metadata.get("policy_spec")
        if raw_spec is None:
            raise InterfaceValidationError(
                "remote server metadata does not contain inferaxis.policy_spec."
            )
