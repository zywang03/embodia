"""Private method-resolution helpers shared by inferaxis mixins."""
...
def resolve_impl(
    ...
    *,
    ...
    """Resolve the wrapped implementation for one public inferaxis method."""
...
def resolve_mapping(owner: object, attr_name: str) -> Mapping[str, str]:
    ...
    if attr_name == "METHOD_ALIASES":
        instance_value = getattr(owner, "_inferaxis_method_aliases", None)
...
def get_modality_map(owner: object, modality: ModalityToken | str) -> Mapping[str, str]:
    ...
    instance_value = getattr(owner, "_inferaxis_modality_maps", None)
    if instance_value is not None:
