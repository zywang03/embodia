"""Payload transforms for inferaxis remote inference."""
...
def _read_action_shape_from_inferaxis_metadata(
    response_or_actions: Mapping[str, Any] | Sequence[Any] | object,
    ...
    inferaxis_meta = response_or_actions.get("inferaxis")
    if not isinstance(inferaxis_meta, Mapping):
        return None, None, None

    target = inferaxis_meta.get("action_target")
    if not isinstance(target, str) or not target.strip():
        target = None

    command = inferaxis_meta.get("action_command")
    if not isinstance(command, str) or not command.strip():
        command = None

    ref_frame = inferaxis_meta.get("action_ref_frame")
    if ref_frame is not None:
        if not isinstance(ref_frame, str) or not ref_frame.strip():
            raise InterfaceValidationError(
                "remote response inferaxis.action_ref_frame must be a non-empty "
...
def _coerce_inferaxis_action_plan(
    plan: Action | Mapping[str, Any] | Sequence[Action | Mapping[str, Any]],
) -> list[Action]:
    """Normalize an inferaxis action plan into a validated list of actions."""
...
def actions_to_action_plan(
    ...
    *,
    ...
    """Convert one remote action chunk into inferaxis-standard actions."""

    metadata_target, metadata_command, metadata_ref_frame = (
        _read_action_shape_from_inferaxis_metadata(response_or_actions)
    )
    ...
    if resolved_target is None:
        raise InterfaceValidationError(
            ...
            "target=..., include inferaxis.action_target in the response, or "
            "use a custom response_to_action callback."
        )
    if resolved_command is None:
        raise InterfaceValidationError(
            "remote action payload does not define a command. Provide "
            "command=..., include inferaxis.action_command in the response, or use "
...
def response_from_action_plan(
    plan: Action | Mapping[str, Any] | Sequence[Action | Mapping[str, Any]],
    *,
    include_inferaxis_metadata: bool = True,
) -> dict[str, Any]:
    """Convert inferaxis actions into one remote response dictionary."""

    actions = _coerce_inferaxis_action_plan(plan)
...
class RemoteTransform:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.include_inferaxis_metadata, bool):
            raise InterfaceValidationError(
                "RemoteTransform.include_inferaxis_metadata must be a bool."
            )

    def build_obs(self, frame: Frame | Mapping[str, Any]) -> dict[str, Any]:
        """Convert one inferaxis frame into one remote observation payload."""
    ...
    def build_frame(self, obs: Mapping[str, Any]) -> Frame:
        """Convert one remote observation mapping into an inferaxis frame."""
    ...
    def first_action_from_response(
        ...
        """Convert one remote response into the first inferaxis action."""
    ...
    def response_from_action_plan(
        ...
        *,
        ...
        """Convert inferaxis actions into one remote response payload."""

        if self.response_builder is None:
            return response_from_action_plan(
                plan,
                include_inferaxis_metadata=self.include_inferaxis_metadata,
            )
        ...
        response = self.response_builder(
            _coerce_inferaxis_action_plan(plan),
            normalized_frame,
        )
